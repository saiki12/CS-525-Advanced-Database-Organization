...
extern RC createBtree (char *indexId, DataType keyType, int n)
{
	...
	openPageFile(indexId, &file_handle);
...
extern RC openBtree (BTreeHandle **tree, char *indexId)
{
	...
	initBufferPool(tree_info->bm,indexId, 10, RS_FIFO, NULL);
...
extern RC deleteBtree (char *indexId)
{
	if(remove(indexId) == 0)
		return RC_OK;
